...
def plot_fig1_fit(report_dir: Path, out_dir: Path) -> None:
    ...
    for i, s_dir in enumerate(slice_dirs):
        ...
        t = np.arange(len(df))
        ax.scatter(t, df["y"], alpha=0.6, s=15, label="Observed Cases")
        ax.plot(t, df["mu"], linewidth=2, label="Model (Fit)")
        ax.set_title(f"{s_dir.name}")
        ax.set_xlabel("Days")
    ...
    fig.suptitle("Figure 1: Model Fit Across Sequential Time Slices", fontsize=16)
    safe_mkdir(out_dir)
